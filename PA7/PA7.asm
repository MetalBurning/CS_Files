...
SHIFT_MANT_RI   ST R2, Param2			; R2 is shift amount for mantissa,
		LD R3, valSum
		ST R3, Param1			; Param1 still contains final mantissa
		JSR right_shift
...
;-----------------------------------------------------------------------------
		...
		LD R2, IMPL1_MASK		; load impl one mask into R2
...
FINISH_ADD	ST R1, norm_shift_amount	; store cleared mantissa in temporary unused memory
		LD R2, TEN
		ST R3, Param1			; expSum
		ST R2, Param2			; 10 shift left ten times
		JSR left_shift
		LD R1, Result
		ST R1, Param1			; shifted exponent
		LD R0, norm_shift_amount	; load cleared mantissa from temporary unused memory
		ST R0, Param2			; valueSUM
...
OR_SIGN_IN	ST R1, Param1			; R1 contains zero
		ST R0, Param2			; R0 contains exp + mantissa or'ed together
		JSR binary_or			; Result is now set
                LD R7,flt16_add_ra   ; restore return address
                RET
;------------------------------------------------------------------------------
flt16_sub       ST R7, sub_return
		LD R4, Param1			; copy of param1
		LD R1, Param2			; Result is Param1 minus Param2
		ST R1, Param1			; store param2 as param1
		JSR flt16_neg
		LD R2, Result			; param2 negated
		ST R4, Param1			; original param1
		ST R2, Param2
		JSR flt16_add
                LD R7, sub_return                  
                RET
